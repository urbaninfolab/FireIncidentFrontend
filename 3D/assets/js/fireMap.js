...
function addForecastedSmoke(map, KMLstring, forecastGroup) {
  ...
  var track = {
    ...
    onAdd: function (map, mbxContext) {
      ...
      var options = {
        obj:
          "https://generated.activefires.ml/" +
          KMLstring.substring(KMLstring.indexOf("data/") + 5),
        type: "fbx",
        scale: 0.006,
        units: "meters",
...
function getYesterday() {
  document.getElementById("CurrentSelectedDate").textContent =
    "⠀ ⠀᠎⠀ ⠀Yesterday";
  var datePicker = document.querySelector(".date-picker");
  datePicker.style.display = "none";
  // clear all markers and rebuild map layer
  ...
  switch (shapefile_display_flag) {
    ...
    case "test-risk-radio":
      ...
      shpfile = new L.Shapefile(shapefileName, {
        onEachFeature: function (feature, layer) {
          var averageNum =
            result[cities[feature.properties["ctid"].substring(0, 5)]];
      ...
      fetch(filePath)
        ...
        .then((data) => {
          ...
          for (var i = 0; i < jsonResult.length; i++) {
            var marker = L.marker([jsonResult[i].lat, jsonResult[i].lon]).addTo(
              map
            );
...
var airMarkers = [];
const PURPLE_AIR_API_KEY = "81D9ACDC-966F-11EC-B9BF-42010A800003";
...
async function populateSensorInformationData(latlng) {
  ...
  purpleAirSensorInformationURL.searchParams.append(
    "api_key",
    PURPLE_AIR_API_KEY
  );
  purpleAirSensorInformationURL.searchParams.append("nwlat", latlng[0]);
  purpleAirSensorInformationURL.searchParams.append("selat", latlng[1]);
  purpleAirSensorInformationURL.searchParams.append("nwlng", latlng[2]);
  purpleAirSensorInformationURL.searchParams.append("selng", latlng[3]);
  purpleAirSensorInformationURL.searchParams.append("fields", fields);

  let response = await fetch(purpleAirSensorInformationURL);
...
async function mapPurpleAirData(map) {
  ...
  let cities = {
    ...
    SanAntonio: [29.590767, 29.249684, -98.679199, -98.27559],
    ...
    for (const city in cities) {
      ...
      sensorList.data.map((sensor) => {
        ...
        let popupData = {
          ...
          altitude: sensor[4],
          "pm2.5": sensor[5],
          "pm2.5_10minute": sensor[6],
          "pm2.5_30minute": sensor[7],
        ...
        var color = popupInformation[0];
        var description = popupInformation[1];
...
function buildAirDataPopup(marker, popupData, description) {
  ...
  airMarkerPopup += buildAirTable(popupData);
...
function getPMDescription(pm10Mins, AQI = false) {
  ...
  if ((AQI && pm10Mins <= 50) || pm10Mins <= 12) {
    color = "#00e400";
    description =
      "<span>0-12: Air quality is satisfactory, and air pollution poses little or no risk with 24 hours of exposure.</span><BR>";
  } else if ((AQI && pm10Mins <= 100) || pm10Mins <= 35) {
    color = "#fdff01";
    description =
      "<span>12.1-35.4: Air quality is acceptable. However, there may be a risk for some people with 24 hours of exposure, particularly those who are unusually sensitive to air pollution.</span><BR>";
  } else if ((AQI && pm10Mins <= 150) || pm10Mins <= 55) {
    color = "#ff7e01";
    description =
      "<span>35.5-55.4: Members of sensitive groups may experience health effects. The general public is less likely to be affected.</span><BR>";
  } else if ((AQI && pm10Mins <= 200) || pm10Mins <= 150) {
    color = "#ff0100";
    description =
      "<span>55.5-150.4: Some members of the general public may experience health effects: members of sensitive groups may experience more serious health effects.</span><BR>";
  } else if ((AQI && pm10Mins <= 300) || pm10Mins <= 250) {
    color = "#8f3f97";
    description =
      "<span>150.5-250.4: Health Alert: The risk of health effects is increased for everyone.</span><BR>";
  } else if ((AQI && pm10Mins <= 301) || pm10Mins <= 350) {
    color = "#7e0023";
    description =
      "<span>250.5-350.4: Health Warning: Health warning of emergency conditions: everyone is more likely to be affected.</span><BR>";
  } else {
    color = "#7e0023";
    description =
      "<span>>=350.5: Health Warning: Health warning of emergency conditions: everyone is more likely to be affected.</span><BR>";
  }
...
async function mapMicrosoftAirData(map) {
  ...
  let airApiUrl =
    "https://eclipseprowebapi.azurewebsites.net/EclipseData/GetEclipseData?CustomerName=Austin&RecentNHours=1";
  ...
  for (let i = 0; i < microData.length; i++) {
    ...
    for (let j = 0; j < microsoftSerialNumber.length; j++) {
      ...
      if (
        serial_number === deviceNum &&
        !addedSerialNumber.has(serial_number)
      ) {
...
function buildMicrosoftAirDataPopup(marker, popupData, description) {
  ...
  var airMarkerPopup = `
            ...
            <span>Temperture: ${(
              Math.round(popupData.TempC * 100) / 100
            ).toFixed(2)} </span><BR>
            <span>Humidity: ${(
              Math.round(popupData.Humidity * 100) / 100
            ).toFixed(2)} </span><BR>


        `;
  airMarkerPopup += `
                <span>AQI: ${(Math.round(popupData.PM10 * 100) / 100).toFixed(
                  2
                )} </span><BR>
...
function buildMicrosoftAirTable(pmData) {
  ...
  var data = `
                <tr>
                    <td>${(Math.round(pmData["PM1"] * 100) / 100).toFixed(
                      2
                    )}</td>
                    <td>${(Math.round(pmData["PM10"] * 100) / 100).toFixed(
                      2
                    )}</td>
                    <td>${(Math.round(pmData["PM25"] * 100) / 100).toFixed(
                      2
                    )}</td>
                </tr>
                `;
...
var data =
  "<item><title>Traffic Injury Pri 4F</title>" +
...
document.getElementsByClassName("mapboxgl-ctrl-attrib")[0].style.display =
  "none";
document.getElementsByClassName("mapboxgl-ctrl-logo")[0].style.display = "none";
document.getElementsByClassName("mapboxgl-ctrl-scale")[0].style.display =
  "none";
